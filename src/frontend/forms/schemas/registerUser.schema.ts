...
import { validDDDs } from '../validations/constants/validDDDs';
import { z } from "zod";
...
export const registerUserSchema = z.object({
  ...
  surname: z.string().superRefine((val, ctx) => {
    ...
    if (!/^[A-Za-zÀ-ÖØ-öø-ÿ]+(?:\s[A-Za-zÀ-ÖØ-öø-ÿ]+)*$/.test(val)) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "O sobrenome deve conter apenas letras e acentuações válidas\n",
      });
    }
  }),
...
email: z.string().superRefine((val, ctx) => {
  ...
  // Validação de caracteres especiais (antes de qualquer split)
  if (!/^[A-Za-z0-9@.]+$/.test(email)) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message: "O e-mail não pode conter caracteres especiais (apenas letras, números, @ e . são permitidos)\n",
    });
  }
  ...
  telephone: z.string().superRefine((val, ctx) => {
    ...
    // Agora verificamos se o DDD tem 2 dígitos (que é o esperado para o Brasil)
    if (rawDDD.length !== 2) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "DDD inválido. O DDD deve ter 2 dígitos válidos.\n",
      });
      return;
    }
  ...
  .superRefine((data, ctx) => {
    ...
    if (!data.name.trim() && !data.surname.trim() && !data.email.trim() && !data.telephone && !data.password?.trim() && !data.confirmPassword?.trim()) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
    ...
    // Validação de name e surname iguais
    if (
      data.name.trim() &&
      data.surname.trim() &&
      ...
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['surname'], // Erro associado à surname
